...
The canonical TRN content shape is documented in `.claude/trn-format-reference.md`. Skim it before starting — it defines the structure for Breaking Changes, Deprecations, and Features entries.
...
When writing new entries, follow the canonical format defined in `.claude/trn-format-reference.md` and the workflow steps in the relevant per-entry skill:

- **Feature entry** — see `.claude/skills/write-trn-feature-section/SKILL.md`. Each entry is a one-paragraph announcement with an `**Automatic**` / `**Configurable**` / `**Automatic / Configurable**` tag and a link to a guide or reference page.
- **Breaking change entry** — see `.claude/skills/write-trn-breaking-change/SKILL.md`. Each entry has a prose intro plus `#### Detect` and `#### Fix` sub-sections.
- **Deprecation entry** — see `.claude/skills/write-trn-deprecation/SKILL.md`. Same structure as breaking changes plus a planned removal release named in the prose intro.

During cleanup you won't have all the inputs those skills normally gather (demo slides, screenshots, Notion requirement, doc links) — write the best entry you can from the PR alone, and for any inputs that are missing but would meaningfully improve the entry, add a checkbox to the PR's `## Open Tasks` list rather than leaving a placeholder in the entry itself.
