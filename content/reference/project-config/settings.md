...
### How to opt-in
Before tracking is enabled for a downstream, an appendum to the Data Processing Agreement has to be signed. Then, Livingdocs can add the corresponding `customerId` to the allow-list which also requires a patch version to be deployed afterward.
