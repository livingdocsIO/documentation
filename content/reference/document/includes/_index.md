...
- how to implement custom `doc-include` user interfaces in the [editor]({{< ref "/reference/document/includes/editor-customization" >}})
...
The [server customization]({{< ref "/customising/server/include-functions" >}}) is normally a simple node.js plugin that uses any templating language (e.g. lodash templates) to render a HTML string from a template and parameters.
The [editor customization]({{< ref "/reference/document/includes/editor-customization" >}}) can be done in 3 different ways: a schema describing the params, a Vue component or an AngularJS component.
