...
In order to actually render content, you need to configure the server to do so, [see here]({{< ref "./server-customization.md" >}}) how that works.
...
[See here]({{< ref "./server-customization.md#generated-sidebar" >}}) for an example.
...
In the [server configuration]({{< ref "./server-customization.md#include-ui-options" >}}) we learned that you can also register an external iframe as a user interface in a modal.
