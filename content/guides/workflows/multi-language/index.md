...
On the server, we first need to provide which languages we want to support in our project. This is defined per [channel]({{< ref "/reference-docs/project-config/project-config.md" >}}). In the configuration file for your channel
add code as follows.
...
The editor side is relatively easy to configure. You only need to add the provided language [display filter]({{< ref "/reference/display_filter.md" >}}) to your dashboard so that you are able to filter documents by language. This is done in the editor environment config file.
