...
It is possible to register a custom filter and use it as a [displayFilter]({{< ref "/reference/display_filter.md" >}}) for dashboards or search modals.
...
Hint: Look into [Filter Query Types]({{< ref "/reference/base_filter.md" >}}) to find possible `{type, value}` combinations for the `filter.options` in the `mount` function.
